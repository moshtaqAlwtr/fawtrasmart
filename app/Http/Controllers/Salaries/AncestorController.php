...
class AncestorController extends Controller
{
    public function index(Request $request)
    {
       ...
       // Calculate global totals for all displayed records
$displayedTotalAmount = $ancestors->sum('amount');
$displayedTotalPaid = 0;
$displayedTotalInstallments = 0;

foreach ($ancestors as $ancestor) {
    $displayedTotalPaid += $ancestor->payments->where('status', 'paid')->sum('amount');
    $displayedTotalInstallments += $ancestor->payments->where('status', 'paid')->count();
}
...
$displayedProgressPercentage = $displayedTotalAmount > 0 ? ($displayedTotalPaid / $displayedTotalAmount) * 100 : 0;
  ...
  $ancestors->each(function ($ancestor) {
        $ancestor->next_payment = $ancestor->payments()
            ->where('status', 'unpaid')
            ->orderBy('due_date')
            ->first();
    });
        return view('salaries.ancestor.index', compact('ancestors', 'paymentRates', 'statuses', 'branches', 'tags', 'employees','account_setting'));
    }
...
public function pay($id)
{
    ...
    // في الكونترولر
$InstallmentPayments = InstallmentPayment::select(
    'id',
    'amount',
    DB::raw("DATE_FORMAT(due_date, '%Y-%m-%d') as due_date"),
    'status'
)->where('salary_advance_id',$id)->where('status', 'unpaid')->get();
      $mainTreasuryAccount = null;
        $user = Auth::user();
...
public function storePayments(Request $request, $id)
{
   ...
   $MainTreasury = null;
        ...
        $journalEntry = JournalEntry::create([
            ...
            'description' => 'سلفية رقم ' . $salaryAdvance->id,
            'status' => 1,
            'currency' => 'SAR',
   ...
   public function show($id)
{
    try {
        ...
        return view('salaries.ancestor.show', compact(
            'ancestor',
            'id',
            'totalPaid',
            'paidInstallments',
