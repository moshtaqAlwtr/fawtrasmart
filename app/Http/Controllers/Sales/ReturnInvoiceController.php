    ...
    try {
        ...
        if ($advance_payment > 0 || $is_paid) {
            ...
            $payment = PaymentsProcess::create([
                ...
                'amount' => $isReturn ? -$payment_amount : $payment_amount, // سالب للمرتجع
                'payment_date' => now(),
                'payment_method' => $request->payment_method,
                'reference_number' => $request->reference_number,
            ...
            // تحديث رصيد الخزينة
            if ($isReturn) {
                $MainTreasury->balance -= $payment_amount; // خصم للمرتجع
            } else {
                $MainTreasury->balance += $payment_amount; // إضافة للفاتورة العادية
            }
            $MainTreasury->save();
