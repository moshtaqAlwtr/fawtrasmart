...
@section('content')
        ...
        <div class="content-header-left col-md-9 col-12 d-flex align-items-center">
            <div class="row breadcrumbs-top flex-grow-1">
                <div class="col-12 d-flex align-items-center">
                    <h2 class="content-header-title mb-0 me-3">تقرير مشتريات المنتجات حسب المنتج</h2>
    ...
    <div class="card">
        <div class="card-header">فلترة تقرير المنتجات</div>
        <div class="card-body">
            <form id="productFilterForm" method="GET" action="{{ route('ReportsPurchases.prodectPurchases') }}">
                <div class="row mb-3">
                    <!-- فلترة حسب المورد -->
                    <div class="col-md-3">
                ...
                <div class="text-center mt-4">
                    ...
                    <a href="{{ route('ReportsPurchases.prodectPurchases') }}" class="btn btn-secondary" id="resetFilter">إلغاء الفلترة</a>
                </div>
            </form>
        </div>
    </div>
            ...
            <tbody>
                @foreach ($groupedInvoices as $productId => $invoices)
                    @php
                        ...
                        $productTotal = $productTotals[$productId] ?? null;
                    @endphp

                    <!-- صف اسم المنتج -->
                    <tr style="background-color: #f8f9fa; font-weight: bold;">
                        <td colspan="11">{{ $product->name }}</td>
                    </tr>
                    ...
                    @foreach ($invoices as $invoice)
                        <tr>
                            ...
                            <td>{{ $invoice->purchaseInvoice->name??"-"  }}</td>
                    ...
                    @if ($productTotal)
                        <tr style="background-color: #e9ecef; font-weight: bold;">
                            <td colspan="8" style="text-align: right;">إجمالي المنتج</td>
                            <td>{{ number_format($productTotal['total_tax'], 2) }}</td>
                            <td>{{ number_format($productTotal['total_quantity'], 2) }}</td>
                            <td>{{ number_format($productTotal['total_amount'], 2) }}</td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
