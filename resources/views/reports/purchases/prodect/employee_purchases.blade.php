...
@section('content')
        ...
        <div class="content-header-left col-md-9 col-12 d-flex align-items-center">
            <div class="row breadcrumbs-top flex-grow-1">
                <div class="col-12 d-flex align-items-center">
                    <h2 class="content-header-title mb-0 me-3">تقرير مشتريات حسب الموظف</h2>
    ...
    <div class="card">
        <div class="card-header">فلترة تقرير الموظف</div>
        <div class="card-body">
            <form id="employeeFilterForm" method="GET" action="{{ route('ReportsPurchases.employeePurchases') }}">
                <div class="row mb-3">
                    <!-- فلترة حسب المورد -->
                    <div class="col-md-3">
                ...
                <div class="text-center mt-4">
                    ...
                    <a href="{{ route('ReportsPurchases.employeePurchases') }}" class="btn btn-secondary" id="resetFilter">إلغاء الفلترة</a>
                </div>
            </form>
        </div>
    </div>
            ...
            <table class="table table-bordered table-striped" id="employeesTable">
                ...
                <tbody>
                    @foreach ($groupedInvoices as $employeeId => $invoices)
                        @php
                            $employee = $invoices->first()->purchaseInvoice->creator->name;
                            $employeeTotal = $employeeTotals[$employeeId] ?? null;
                        @endphp

                        <!-- صف اسم الموظف -->
                        <tr style="background-color: #f8f9fa; font-weight: bold;">
                            <td colspan="11">{{ $employee }}</td>
                        </tr>
                        ...
                        @foreach ($invoices as $invoice)
                            <tr>
                                ...
                                <td>{{ $invoice->invoice->employee->full_name ?? 'غير معروف' }}</td>
                        ...
                        @if ($employeeTotal)
                            <tr style="background-color: #e9ecef; font-weight: bold;">
                                <td colspan="8" style="text-align: right;">إجمالي الموظف</td>
                                <td>{{ number_format($employeeTotal['total_tax'], 2) }}</td>
                                <td>{{ number_format($employeeTotal['total_quantity'], 2) }}</td>
                                <td>{{ number_format($employeeTotal['total_amount'], 2) }}</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
