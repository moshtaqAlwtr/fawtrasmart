...
@section('content')
        ...
        <div class="content-header-left col-md-9 col-12 d-flex align-items-center">
            <div class="row breadcrumbs-top flex-grow-1">
                <div class="col-12 d-flex align-items-center">
                    <h2 class="content-header-title mb-0 me-3">تقرير الحركة التفصيلية للمخزون </h2>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="inventoryForm" method="GET" action="{{ route('StorHouseReport.detailedMovementInventory') }}">
                <div class="row mb-3">
                    <!-- التصنيف -->
                    <div class="col-md-3">
                ...
                <div class="text-center mt-4">
                    ...
                    <a href="{{ route('StorHouseReport.detailedMovementInventory') }}" class="btn btn-secondary"
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach($movements as $movement)
                        <tr>
                            <td>{{ $movement->created_at->format('H:i d/m/Y') }}</td> <!-- الوقت -->
                            <td>{{ $movement->type_of_operation }}</td> <!-- النوع -->
                            <td>{{ $movement->product->serial_number ?? 'N/A' }}</td> <!-- كود المنتج -->
                            <td>{{ $movement->product->name ?? 'N/A' }}</td> <!-- المنتجات -->
                            <td>{{ $movement->type_of_operation === 'in' ? $movement->quantity : 0 }}</td> <!-- الوارد -->
                            <td>{{ $movement->type_of_operation === 'out' ? $movement->quantity : 0 }}</td> <!-- المنصرف -->
                            <td>{{ $movement->comments ?? 'N/A' }}</td> <!-- ملاحظات -->
                            <td>{{ $movement->storeHouse->name ?? 'N/A' }}</td> <!-- المستودع -->
                        </tr>
                    @endforeach
                </tbody>
            </table>
