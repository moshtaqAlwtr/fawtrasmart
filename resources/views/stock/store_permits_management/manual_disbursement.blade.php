                            ...
                            <div class="form-body row">
                                <div class="form-group col-md-2 mt-1">
                                    <div class="vs-checkbox-con vs-checkbox-primary">
                                        <input type="checkbox" name="permission_type" checked="" value="2" disabled>
                                        <span class="vs-checkbox">
                                            <span class="vs-checkbox--check">
                                                <i class="vs-icon feather icon-check"></i>
                                            </span>
                                        </span>
                                        <span class="">إذن صرف يدوي</span>
                                    </div>
                                </div>

                                <input type="hidden" name="permission_type" value="2">
...
@section('scripts')
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let selectedPermissionType = 2; // نوع الإذن الافتراضي
            ...
            // دالة لتحديث البنود بناءً على المخزن الجديد
            function updateItemsStock() {
                ...
                rows.forEach(row => {
                    ...
                    if (productId) {
                        // جلب الكمية الحالية للبند من المخزن الجديد
                        fetch(`/get-product-stock/${selectedStoreId}/${productId}`)
                            .then(response => response.json())
                            .then(data => {
                                if (data.stock !== null) {
                                    const stockBefore = data.stock; // الكمية الحالية في المخزن الجديد
                                    row.querySelector('.stock-before').textContent = stockBefore;

                                    // تحديث "رصيد المخزن بعد" بناءً على الكمية المدخلة ونوع الإذن
                                    const quantity = parseInt(row.querySelector('.quantity').value, 10) || 0;
                                    updateStockAfter(row, stockBefore, quantity);
                                } else {
                                    // إذا كان البند غير موجود في المخزن الجديد
                                    if (selectedPermissionType === 1) {
                                        // في حالة الإضافة، يتم تعيين "رصيد المخزن قبل" إلى 0
                                        row.querySelector('.stock-before').textContent = "0";
                                        updateStockAfter(row, 0, 1); // رصيد المخزن بعد الإضافة
                                    } else {
                                        // في حالة الصرف أو التحويل، يتم إظهار رسالة خطأ
                                        Swal.fire({
                                            icon: 'error',
                                        ...
                                        row.querySelector('.stock-before').textContent = "0";
                                        row.querySelector('.stock-after').textContent = "0";
                                    }
                                }
                            });
                    }
            ...
            itemsTable.addEventListener('change', function (e) {
                if (e.target.classList.contains('product-select')) {
                    if (!selectedStoreId) {
                        // alert("يرجى اختيار المخزن أولاً قبل اختيار البند.");
                        Swal.fire({
                            icon: 'warning',
                    ...
                    .then(data => {
                        if (data.stock !== null) {
                            const stockBefore = data.stock; // الكمية الحالية في المخزن
                            row.querySelector('.stock-before').textContent = stockBefore;
                            ...
                            // إذا كان البند غير موجود في المخزن
                            if (selectedPermissionType === 1) {
                                // في حالة الإضافة، يتم تعيين "رصيد المخزن قبل" إلى 0
                                row.querySelector('.stock-before').textContent = "0";
                                updateStockAfter(row, 0, 1); // رصيد المخزن بعد الإضافة
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        row.remove(); // حذف الصف بعد تأكيد المستخدم
                                        updateGrandTotal(); // تحديث الإجمالي الكلي
                                    }
                                });
                            }
                        }
            ...
            // دالة لحساب "رصيد المخزون بعد" بناءً على نوع الإذن
