        ...
        <div class="container" style="max-width: 1200px">
            <div class="card">
                <div class="card-title p-2">
                    <a href="{{ route('shift_management.edit',$shift->id) }}" class="btn btn-outline-primary btn-sm">تعديل <i class="fa fa-edit"></i></a>
                    <a href="#" class="btn btn-outline-danger btn-sm" data-toggle="modal" data-target="#modal_DELETE{{ $shift->id }}">حذف <i class="fa fa-trash"></i></a>
                                        ...
                                        @foreach ($shift->days as $key)
                                            <table class="table">
                                                <thead style="background: #f8f8f8">
                                                    <tr>
                                                        <th>يوم : <small>( {{ $arabicDays[$key->day] ?? $key->day }} )</small></th>
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td>
                                                            <p><small>بداية الوردية </small>: <strong>{{ $key->start_time }}</strong></p>
                                                            <p><small>بداية تسجيل الدخول </small>: <strong>{{ $key->login_start_time }}</strong></p>
                                                            <p><small>نهاية تسجيل الدخول </small>: <strong>{{ $key->login_end_time }}</strong></p>
                                                            <p><small>فترة السماح </small>: <strong>{{ $key->grace_period }} دقيقة</strong></p>
                                                        </td>
                                                        <td>
                                                            <p><small>نهاية الوردية </small>: <strong>{{ $key->end_time }}</strong></p>
                                                            <p><small>بداية تسجيل الخروج </small>: <strong>{{ $key->logout_start_time }}</strong></p>
                                                            <p><small>نهاية تسجيل الخروج </small>: <strong>{{ $key->logout_end_time }}</strong></p>
                                                            <p><small>حساي التأخير </small>: <strong>@if($key->delay_calculation == 1)  بعد موعد بداية الوردية + مهلة التأخير  @else من موعد بداية الوردية @endif</strong></p>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
