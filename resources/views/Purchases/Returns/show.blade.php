...
@section('title')
مرتجع مشتريات #{{ $purchaseInvoiceReturn->code }}
@stop
    ...
    <div class="content-header d-flex justify-content-between align-items-center mb-2">
        <div>
            <h2 class="content-header-title mb-0">
                مرتجع مشتريات {{ $purchaseInvoiceReturn->code }}
                <span class="badge {{ $purchaseInvoiceReturn->status == 1 ? 'bg-warning' : 'bg-success' }}">
                    {{ $purchaseInvoiceReturn->status == 1 ? 'معلق' : 'مكتمل' }}
                </span>
            </h2>
            <small>المورد: {{ $purchaseInvoiceReturn->supplier->name }} - رقم المرتجع #{{ $purchaseInvoiceReturn->code }}</small>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header p-2">
            <div class="d-flex gap-2">
                ...
                <div class="btn-group">
                    <a href="{{ route('ReturnsInvoice.edit', $purchaseInvoiceReturn->id) }}" class="btn btn-outline-primary btn-sm">
                                        ...
                                        <div class="card shadow" style="max-width: 600px; margin: 20px auto;">
                                            <div class="card-body bg-white p-4" style="min-height: 400px; overflow: auto;">
                                                <div style="transform: scale(0.8); transform-origin: top center;">
                                                    @include('Purchases.Returns.pdf', ['purchaseInvoiceReturn' => $purchaseInvoiceReturn])
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>هل أنت متأكد من حذف مرتجع المشتريات رقم "{{ $purchaseInvoiceReturn->code }}"؟</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">إلغاء</button>
                    <form action="{{ route('ReturnsInvoice.destroy', $purchaseInvoiceReturn->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
