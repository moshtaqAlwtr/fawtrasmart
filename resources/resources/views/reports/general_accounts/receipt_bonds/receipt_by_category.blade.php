...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0"> تقرير سندات القبض حسب التصنيف
                    </h2>
    ...
    <div class="container mt-5">
        ...
        <div class="card p-4 mb-3">
            <form action="{{ route('GeneralAccountReports.ReceiptByCategory') }}" method="GET">
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-3">
                        <label for="inputGroupBy" class="form-label">التصنيف:</label>
                        <select class="form-control" id="inputGroupBy" name="incomes_category">
                            <option value="">اختر التصنيف</option>
                            @foreach ($receiptsCategory as $category)
                                <option value="{{ $category->id }}"
                                    {{ request('incomes_category') == $category->id ? 'selected' : '' }}>
                ...
                <a href="{{ route('GeneralAccountReports.ReceiptByCategory') }}" class="btn btn-custom mt-3">الغاء
                    الفلتر</a>
            </form>
        </div>
        ...
        <div class="card p-4 mb-3">
            ...
            <!-- Chart Section -->
            <div class="card p-4 mb-3">
                <h5>مخطط الإجمالي حسب التصنيف</h5>
            ...
            <div id="summaryTable">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        @foreach ($groupedReceipts as $categoryId => $receiptsInCategory)
                            @php
                                $category = $receiptsInCategory->first()->incomes_category;
                            @endphp

                            <!-- عرض التصنيف -->
                            <tr style="background-color: #f8f9fa;">
                                <td colspan="12"><strong>{{ $category->name ?? 'غير مصنف' }}</strong></td>
                            </tr>

                            <!-- عرض التفاصيل تحت التصنيف -->
                            @foreach ($receiptsInCategory as $receipt)
                                <tr>
                                    <td>{{ $receipt->code }}</td>
                                    <td>{{ $receipt->date }}</td>
                            ...
                            <!-- عرض المجموع لكل تصنيف -->
                            <tr style="background-color: #e9ecef;">
                                <td colspan="9"><strong>المجموع</strong></td>
                                <td><strong>{{ number_format($receiptsInCategory->sum('amount'), 2) }}</strong></td>
                                <td><strong>{{ number_format($receiptsInCategory->sum('tax1_amount') + $receiptsInCategory->sum('tax2_amount'), 2) }}</strong>
                                </td>
                                <td><strong>{{ number_format($receiptsInCategory->sum('amount') + $receiptsInCategory->sum('tax1_amount') + $receiptsInCategory->sum('tax2_amount'), 2) }}</strong>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
@section('scripts')
    ...
    <script>
        // بيانات المخطط
        const chartData = {
            labels: @json($chartData['labels']), // أسماء التصنيفات
            datasets: [{
                label: 'الإجمالي',
